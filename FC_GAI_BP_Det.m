function symest = FC_GAI_BP_Det(TxAntNum,RxAntNum,slen,RxSymbol,H,Nv,sym,delta,iterNum)
symp=permute(sym,[2 3 1]);
xest=(H'*H+Nv*eye(2*TxAntNum)/2)\H'*RxSymbol;
temp=xest-sym';
Px=repmat(permute(~(temp-min(temp,2)),[1 3 2]),1,2*RxAntNum,1);
